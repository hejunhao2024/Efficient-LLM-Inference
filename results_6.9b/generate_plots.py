...
def parse_float_list(x):
    """
    解析类似 "[7.924786, 8.53902, ...]" 的字符串为 float list。
    如果已经是 list，也能处理。
    """
    if pd.isna(x):
        return []
...
def prepare_df(csv_path: str) -> pd.DataFrame:
    ...
    # 新格式优先使用 window_ppls 的 5 次结果求平均
    # 如果没有 window_ppls，则退回使用原来的 ppl 列
    if "window_ppls" not in df.columns and "ppl" not in df.columns:
        required_cols.append("window_ppls")
    ...
    if "window_ppls" in df.columns:
        ...
        # 关键改动：用 5 次 window_ppls 的平均值作为最终 ppl
...
def plot_one_metric(
    ...
    title = f"{dataset_name}: {metric_label} vs KV Budget"
    if metric_col == "ppl":
        title += " (mean of 5 windows, knorm_online hidden)"
    plt.title(title)
    ...
    out_path = output_dir / f"{safe_dataset_name}_{metric_col}.png"
